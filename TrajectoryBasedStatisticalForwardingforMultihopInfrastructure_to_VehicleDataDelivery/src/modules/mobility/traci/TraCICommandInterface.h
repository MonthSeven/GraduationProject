...
//		Bien Aime  induction Loop variable
		std::list<std::string> getDetectorsList();
		void getinductionlooplaneID();
		int getLastStepVehicleNumber(std::string detectorId);
		int getLastStephaltingVehiclesNumber(std::string detectorId);
		double getLastStepTime(std::string edgeId);
//		E1 start
		std::list <std::string> getInductionLoopIds();
		double getInductionLoopPosition(std::string InductionLoopId);
		int getInductionLoopCount();
		int getLastStepVehicleNumberE1(std::string InductionLoopId);
		int getLastStepVehicleIds(std::string InductionLoopId);
		double getLastStepMeanSpeed(std::string InductionLoopId);
		std::string getInductionLoopLaneIdE1(std::string InductionLoopId);


//		E1 End
//		std::list <std::string> getEdgesList();
//		end induction loop variables

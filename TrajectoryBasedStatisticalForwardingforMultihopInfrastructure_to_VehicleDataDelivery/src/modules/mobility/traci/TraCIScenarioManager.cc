...
void TraCIScenarioManager::initialize(int stage) {
  ...
  cc = FindModule<ConnectionManager*>::findGlobalModule();
...
//  Added codes for selecting the random edges to vehicles

  std::string in_edge;
  ifstream inf("save_edge.txt");
  while(!inf.eof())
  {
      inf >> in_edge;
      arr_edge.push_back(in_edge);
  }
...
//  Added for dynamic edges assignment

  string veh, current_veh, last_edge, vehicle_last_edge, current_vehicle_edge;
      int flag = 0;

      srand((unsigned)time(NULL));
      veh = objectId_resp;
      if (veh.compare("") != 0){
          if (arr_vehicle.size() == 0){

              list<string> list = commandIfc->getPlannedEdgeIds(veh);// getPlannedEdgeIds(veh); //total edge of vehicle
              last_edge = list.back(); //last edge of vehicle
              arr_vehicle.insert(std::pair<string, string>(veh, last_edge));
              arr_flag_vehicle.insert(std::pair<string, int>(veh, 0));
          }

          FindIter1 = arr_vehicle.find(veh); //search vehicle about last edge
          if (FindIter1 != arr_vehicle.end()) //vehicle being on road
          {
              vehicle_last_edge = FindIter1->second;

              current_vehicle_edge = commandIfc->getEdgeId(veh);

              if (current_vehicle_edge.compare(vehicle_last_edge) == 0) //current_vehicle_edge equal vehicle_last_edge
              {
                  FindIter2 = arr_flag_vehicle.find(veh);

                  flag = FindIter2->second;
                  //cout<<"?"<<endl;
                  if (flag == 0) //flag 0 : reroute
                  {
                      int rand_num; string select_edge;
//                      rand_num = rand() % RANDOMNUMBER;//previous
                      rand_num = intrand(RANDOMNUMBER);
//                      std::cout<<"==== The random number is ====  "<<rand_num<<endl;
//                      std::cout<<"______ The edge array size is ____  "<<arr_edge.size()<<endl;
                      select_edge = arr_edge[rand_num]; //random select reroute edge
//                      std::cout<<"---- The selected edge is ----  "<<select_edge<<endl;
                      //select_edge="190478684#0";//using test!!!!!!!!!!!!!!!!!!!!

                      if (select_edge.compare(vehicle_last_edge) != 0) //random select reroute edge not equal previous vehicle_last_edge
                      {
                          commandIfc->newRoute(veh, select_edge); //route change
                          arr_vehicle[FindIter2->first] = select_edge;
                          arr_flag_vehicle[FindIter2->first] = 1;
                      }
                      else
                      {
                          string add_select_edge_1 = arr_edge[0]; //random select reroute edge

                          //add_select_edge_1="-190478684#2";//using test!!!!!!!!!!!!!!!!!!!!

                          if (select_edge.compare(add_select_edge_1) != 0)
                          {
                              commandIfc->newRoute(veh, add_select_edge_1); //route change
                              arr_vehicle[FindIter2->first] = add_select_edge_1;
                              arr_flag_vehicle[FindIter2->first] = 1;
                          }
                          else
                          {
                              string add_select_edge_2 = arr_edge[1];
                              commandIfc->newRoute(veh, add_select_edge_2); //route change
                              arr_vehicle[FindIter2->first] = add_select_edge_2;
                              arr_flag_vehicle[FindIter2->first] = 1;
                          }
                      }
                  }
              }
              else//current_vehicle_edge not equal vehicle_last_edge
              {
                  FindIter2 = arr_flag_vehicle.find(veh);
                  arr_flag_vehicle[FindIter2->first] = 0;
              }
          }
          else//vehicle not being on road
          {
              list<string> list = commandIfc->getPlannedEdgeIds(veh); //total edge of vehicle
              last_edge = list.back(); //last edge of vehicle
              arr_vehicle.insert(std::pair<string, string>(veh, last_edge));
              arr_flag_vehicle.insert(std::pair<string, int>(veh, 0));
          }
...
//  End of added for dynamic edges assignment

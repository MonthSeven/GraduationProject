...
void Mac1609_4::initialize(int stage)
{
	...
	{
	    /*
		// get vehicle mobility information
	    //veins::TraCIMobility* VehMobi;
	    //	VehMobi = (veins::TraCIMobility*)getOwner()->getOwner()->findObject("veinsmobility", true);
        //
		// because rsu has not Mobility pointer, so first need to exclude rsu
	    //	if(strcmp(findHost()->getName(), "rsu") != 0)
		//{
		    // print out the host name, which is the node name:
	    //	    std::cout << "VehiMobi: " << findHost()->getFullName() << std::endl;
		    // print various mobility information
	    //	    std::cout << "  --> " << "Position: " << VehMobi->getCurrentPosition().x << ", " << VehiMobi->getCurrentPosition().y << std::endl;
	    //	    std::cout << "  --> " << "Speed:    " << VeMobi->getSpeed() << std::endl;
	    //	    std::cout << "  --> " << "RoadID:   " << VehMobi->getRoadId() << std::endl;
		//}
		*/
		// Mac1609_4::Mac80211pToPhy11pInterface* phy11p
	    phy11p = FindModule<Mac80211pToPhy11pInterface*>::findSubModule(getParentModule());
		assert(phy11p); // phy11p가 존재 false면, 실행중단.
...
//		std::cout <<" I am  in itialization : "<< findHost()->getFullName()<<endl;
		DBG <<" I am  in itialization : "<< findHost()->getFullName()<<endl;
...
void Mac1609_4::handleSelfMsg(cMessage* msg)
{
	...
	{
//	    std::cout<<" Next Mac Event Scheduled: "<<endl;
		...
		kwon* pktToSend = myEDCA[activeChannel]->initiateTransmit(lastIdle);
...
{
//    std::cout <<" HandldeUpperMsg : Pkt"<<endl;
	...
	*/
    if ((thisMsg = dynamic_cast<kwon*>(msg)) == NULL)
    {
        error("Mac only accepts KwonMessages");
    }
 ...
 */
int Mac1609_4::EDCA::queuePacket(t_access_category ac,kwon* msg) {
...
*/
kwon* Mac1609_4::EDCA::initiateTransmit(simtime_t lastIdle) {
